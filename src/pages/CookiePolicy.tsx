        ...
        <div className="container mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-8 space-y-6">
          <div className="border border-black/20 bg-white/50 backdrop-blur-sm p-6 md:p-7 relative overflow-hidden">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-[10px] uppercase tracking-[0.35em] font-bold text-black/70">Cookie Policy</span>
            </div>

            <div className="space-y-4">
              {sections.map((section, index) => (
                <div key={section.title} className={`border border-black/15 bg-white/70 px-4 py-4 md:px-5 md:py-5 ${index !== sections.length - 1 ? "mb-3" : ""}`}>
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-1 h-1 bg-blue-500/60" />
                    <h2 className="text-[10px] uppercase tracking-[0.35em] font-bold text-black/75">{section.title}</h2>
                  </div>
          ...
          <div className="border border-black/20 bg-white/50 backdrop-blur-sm p-6 md:p-7 relative overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                ...
                className="flex-1 py-3 text-[9px] uppercase tracking-[0.35em] border border-black/20 bg-white/80 text-black/70 hover:border-black/40 hover:text-black transition-all rounded-none"
              >
                Reset Consent
              </button>
              <a
                href="mailto:privacy@nirakara.studio"
                className="flex-1 py-3 text-[9px] uppercase tracking-[0.35em] border border-black/20 bg-white/80 text-black/70 hover:border-black/40 hover:text-black transition-all text-center rounded-none"
              >
